...
const String scriptFileName = "script.bin"; // Updated to read the binary file
...
void executeScript() {
  scriptFile = SD.open(scriptFileName, FILE_READ);
  ...
  while (scriptFile.available()) {
    // Read each byte and decode it
    char byte = scriptFile.read();
    char decodedByte = byte ^ 0xA5; // Reverse XOR operation (0xA5 is used in encoding)
    
    processByte(decodedByte); // Function to handle each byte
  }
...
void processByte(char decodedByte) {
  static String currentLine = "";

  if (decodedByte == '\n') {
    executeCommand(currentLine);
    currentLine = "";
  } else if ((int)decodedByte != 13) {
    currentLine += decodedByte;
  }
}
